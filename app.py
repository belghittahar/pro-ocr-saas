...
# -----------------------------------------------------------------------------
# AdSense ads.txt Tornado Handler Injection
# -----------------------------------------------------------------------------
class AdsTxtHandler(tornado.web.RequestHandler):
    def get(self):
        self.set_header("Content-Type", "text/plain")
        self.write("google.com, pub-3336794319990692, DIRECT, f08c47fec0942fa0")

def inject_ads_txt():
    # Only run if a server instance exists (prevents issues during `streamlit run` initialization)
    if hasattr(server, 'Server') and hasattr(server.Server, 'get_current'):
        current_server = server.Server.get_current()
        if current_server and hasattr(current_server, '_app'):
            app = current_server._app
            # Check if we already injected the handler to prevent duplicates on reruns
            try:
                # Iterate through router rules to find if /ads.txt is already registered
                handler_exists = any(
                    isinstance(handler, tornado.web.URLSpec) and handler.regex.pattern == '/ads\\.txt$' 
                    for handler in app.default_router.rules
                )
                if not handler_exists:
                    app.add_handlers(r".*", [(r"/ads\.txt", AdsTxtHandler)])
            except Exception:
                pass
